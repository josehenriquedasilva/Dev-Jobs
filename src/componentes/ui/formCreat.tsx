...
export default function FormCreat() {
  ...
  const createVaga = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    setLoading(true);
    setErro("");

    try {
      const response = await fetch("/api/user/createVaga", {
        method: "POST",
        body: JSON.stringify({
          titulo,
          atuacao,
  ...
  return (
    <form onSubmit={createVaga} className="flex flex-col space-y-6">
      <label
        htmlFor="titulo"
